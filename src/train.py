...
from data_utils import LeavesData, train_transform, test_transform,getData
...
class ModelUpdater():
    ...
    def train_one_epoch(self,model):
        ...
        for idx,(imgs,labels) in enumerate(tqdm(self.train_loader)):
            ...
            self.optimizer.step()
            with torch.no_grad():
                metric.add(loss * imgs.shape[0], d2l.accuracy(out, labels), imgs.shape[0])
            if self.args.verbose and (idx + 1) % 10 == 0:
                train_l = metric[0] / metric[2]
                train_acc = metric[1] / metric[2]
                print("{}/{} loss:{}  acc:{}".format(i,len(self.train_loader),train_l,train_acc))
...
if __name__=='__main__':
    ...
    train_loader=dataloader.DataLoader(train_data,args.batch_size,shuffle=True)
